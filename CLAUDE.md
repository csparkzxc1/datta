...
### 8.4 발송 — 자녀에게 어떻게 닿는가 (★ 핵심 디자인 결정)

이 섹션은 *닿다의 정체성*을 결정한다. "이메일로 보낸다"는 *기술적 답*이지만, 닿다의 진짜 산출물은 *의례*다. 자녀 결혼식 전날, 자녀가 부모와 함께 앉아서 *15년 전 부모가 쓴 편지*를 폰으로 함께 여는 그 5분. 그게 진짜 산출물이다. 이메일 한 통이 아니다.

#### 8.4.1 V1 (MVP) — 부모를 통한 의례적 전달
...
그날 unlock_at에 도달한 캡슐 → is_delivered = true 처리
  ↓
작성자(부모)에게 푸시 알림:
"오늘 [자녀]에게 [N]년 전 봉인한 캡슐이 도착했어요"
  ↓
부모가 앱을 열어 캡슐 확인
  ↓
"함께 열어보기" 버튼 → 부모가 자녀와 *함께* 여는 의례
  ↓
캡슐이 "열림" 섹션으로 이동
  ↓
백업: 부모의 등록 이메일로 PDF 사본 자동 발송 (보관용)
```

**V1에서 자녀에게 직접 발송 *안 하는* 이유:**

- 0~10세 자녀는 이메일이 없거나 부모가 관리
- 20년 후 자녀 이메일이 유효할지 불확실 (평균 7년 수명)
- "가족 외 누구도 못 봄" §10 약속과 이메일이 충돌 (구글·네이버가 봄)
- 가장 중요: *함께 여는 의례*가 사라짐. 단순 전달이 아니라 *가족 사건*이어야 함

#### 8.4.2 자녀 나이별 발송 시나리오

발송 방식은 자녀 나이에 따라 자동으로 다르게 처리해야 한다:

|시점      |자녀 상태|V1 발송 방식|V2 발송 방식            |누가 여는가           |
|--------|-----|--------|--------------------|-----------------|
|백일 / 돌  |영아   |부모 알림만  |동일                  |부모 단독            |
|5세 생일   |글 모름 |부모 알림   |동일                  |부모가 읽어줌          |
|초등 입학   |글 읽음 |부모 알림   |부모 알림 + 자녀 옵션       |**부모와 함께**       |
|수능 D-100|청소년  |부모 알림   |부모 + 자녀 본인          |자녀 단독 가능         |
|만 18세   |성년   |부모 알림   |자녀 본인 (1차) + 부모 (백업)|자녀 단독            |
|결혼식 전날  |독립 성인|부모 알림   |자녀 본인 직접            |**자녀 단독** (가장 사적)|

#### 8.4.3 V2 — 자녀 본인 계정 + 멀티 채널 백업
...
부모에게 "자녀 계정 활성화" 안내
  ↓
부모가 자녀에게 가입 초대 발송 (카톡 링크 등)
  ↓
자녀가 닿다 본인 계정 생성 (카카오 / 애플 / 이메일)
  ↓
부모가 자녀를 "수신자"로 연결 (양방향 동의 필요)
  ↓
이후 봉인된 캡슐은 발송 채널 자동 결정:
  - 만 14~17세: 부모 1차, 자녀 2차 (둘 다 알림)
  - 만 18세+: 자녀 1차, 부모 2차
...
발송 채널 (V2, 다중 백업):
  ...
  2순위: 자녀 이메일 (자녀가 등록한)
  3순위: 자녀 SMS (선택)
  백업: 부모 앱 푸시 + 부모 이메일 PDF (영구 보관)
```

#### 8.4.4 V3 — 부모 부재 시 상속 시나리오 (§10의 진짜 의미)

```
시나리오: 부모가 자녀 19세 전에 사망

조건 1: 부모가 6개월간 앱 미접속 + 결제 갱신 실패
  → 상속 관리자에게 "확인 요청" 이메일 + SMS (7일 간격 3회)
  → 7일 내 응답 없으면 부모 상태 "휴면" 처리

상속 관리자 작동:
  → 상속 관리자가 "인계 시작" 클릭
  → 자녀의 신원 확인 (만 19세 이상 + 카카오 본인인증)
  → 자녀에게 모든 봉인 캡슐 인계
  → 자녀가 부모 대신 캡슐 관리 (단, 새 캡슐 작성은 불가)

최후 수단: 회사도 종료 시
  → §10.3 "회사 해산 자동 발송" 약정 발동
  → 모든 봉인 캡슐을 작성자의 등록 이메일로 PDF + ZIP 발송
  → 발송 비용은 가입 시 적립된 보증금에서 차감
  → 6개월 grace period 후 데이터 영구 삭제
```

#### 8.4.5 솔직한 한계 — 20년 약속의 진짜 어려움

**이메일은 사라지고, 폰은 바뀌고, 앱은 망한다.** 이 사실을 사용자에게 *숨기지 않는다*.

대응 전략:

1. **부모가 직접 전하는 게 가장 신뢰성 높음** — V1의 부모 의례 모델이 *기술이 아니라 인간*에 기댄 가장 안전한 답
1. **다중 채널 백업** — V2에서 앱 + 이메일 + SMS + PDF 동시 발송
1. **물리 백업 옵션 (V3)** — 봉인 시점에 *USB·인쇄 책자* 추가 옵션 (Storyworth 모델, +99,000원). 부모가 직접 자녀에게 물리적으로 전달
1. **영구 PDF 내보내기** — §10.1의 가장 중요한 보장. 사용자가 *언제든* USB로 백업
1. **온보딩에서 명시적으로 설명** — "닿다는 부모님이 자녀와 *함께 여는 의례*를 위해 설계되었습니다. 자녀 본인 직접 수령은 만 14세 이후 V2에서 추가됩니다." 이 안내가 *기대 관리*

#### 8.4.6 구현 시 주의사항 (Claude Code에게)

- **V1 MVP에서는 무조건 부모 알림만** — 자녀 직접 발송 기능은 V2까지 보류
- **알림 카피는 §13 톤** 준수:
  - ❌ "도착 알림!" (감탄형 금지)
  - ❌ "캡슐이 만료되었어요" (만료 X, 도착이다)
  - ✅ "오늘, [자녀]에게 [N]년 전 봉인한 캡슐이 닿았어요"
- **함께 열어보기 UX** — 발송 알림 탭 시 *바로* 본문을 보여주지 말고, "함께 열어볼 준비가 되셨나요?" 한 단계 더 거침. 자녀를 옆에 부를 시간 필요
- **음성으로 읽어주기 기능 (V2)** — 자녀가 어리거나 글을 모를 때 부모가 본문을 *읽어줄 수 있도록* 큰 글자 모드 제공
- **이메일 백업은 봉인 시점에 부모 이메일 확정** — 봉인 후 부모 이메일 변경되어도 *원래 이메일*에 우선 발송 (이메일은 영구 보관용 백업이지 1차 전달 수단 아님)
